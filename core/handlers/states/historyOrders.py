...
from core.database.ramarket_shop.db_shop import create_excel_by_shop
...
from core.utils.callbackdata import Country, City, Shops, Org
...
async def select_org(call: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(HistoryOrders.org)
...
async def history_period(call: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(HistoryOrders.start_date)
...
async def start_period(message: Message, state: FSMContext):
    ...
    if len(date) == 3:
        ...
        await state.set_state(HistoryOrders.end_date)
